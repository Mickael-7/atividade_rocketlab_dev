  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500">
        <Link to="/pedidos" className="hover:text-indigo-600 transition-colors">Pedidos</Link>
        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
        </svg>
        <span className="font-mono text-gray-700">{pedido.id_pedido.slice(0, 8)}…</span>
      </nav>

      {/* cabeçalho */}
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-900 font-mono">{pedido.id_pedido}</h1>
          <p className="text-sm text-gray-500 mt-0.5">Comprado em {formatDate(pedido.pedido_compra_timestamp)}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Consumidor</h2>
          <Link
          ...
          <p className="text-sm text-gray-500 mt-1">{pedido.consumidor.cidade}, {pedido.consumidor.estado}</p>
        </div>

        {/* entrega */}
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Entrega</h2>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <span className="text-sm font-bold text-gray-900">{formatBRL(pedido.valor_total)}</span>
        </div>
          ...
          {pedido.itens.map((item) => (
            <div key={item.id_item} className="px-5 py-3 flex items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-0.5">Vendedor: {item.nome_vendedor}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="text-sm font-semibold text-gray-900">{formatBRL(item.preco_BRL)}</p>
                <p className="text-xs text-gray-400">Frete: {formatBRL(item.preco_frete)}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {pedido.avaliacoes.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Avaliações</h2>
          <div className="space-y-4">
            {pedido.avaliacoes.map((av, i) => (
              <div key={i} className="flex gap-3">
                <div className="flex shrink-0 mt-0.5">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <svg key={s} className={`h-4 w-4 ${s <= av.avaliacao ? "text-amber-400" : "text-gray-200"}`} viewBox="0 0 20 20" fill="currentColor">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                  ))}
                </div>
                <div>
                  {av.titulo_comentario && <p className="text-sm font-medium text-gray-900">{av.titulo_comentario}</p>}
                  {av.comentario && <p className="text-sm text-gray-500 mt-0.5">{av.comentario}</p>}
                </div>
              </div>
            ))}
          </div>
